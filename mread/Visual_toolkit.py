...
def load_point_plot(coords, data):
    ...
    mesh["density"] = data
    mesh.plot(point_size = 1, screenshot = 'density.png')




def convert_simplified_array(fieldline):
    # convert the array of vertices to xyz coords
    # use this to convert make_simp to cartes (which we can't use hex_conn for)
    import numpy as np

    simplified_array = make_simplified_array(fieldline)
    rfd('fieldline12300.bin')
    data = rho.flatten()
